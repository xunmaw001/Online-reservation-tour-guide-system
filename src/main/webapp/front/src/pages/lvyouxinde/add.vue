...
<div :style='{"width":"100%","padding":"40px 7%","margin":"0 auto","position":"relative","background":"none"}'>
    <el-form
      ...
      label-width="150px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0","background":"none"}' label="标题" prop="biaoti">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0","background":"none"}' label="简介" prop="jianjie">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="简介"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0","background":"none"}' label="内容" prop="neirong">
            <editor 
...
<script>
  export default {
      ...
      init(type) {
        ...
        this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            ...
            if((json.daoyouzhenghao!=''&&json.daoyouzhenghao) || json.daoyouzhenghao==0){
                this.ruleForm.daoyouzhenghao = json.daoyouzhenghao
            }
            if((json.daoyouxingming!=''&&json.daoyouxingming) || json.daoyouxingming==0){
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('lvyouxinde/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
                  ...
                  params: params
                     ...
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('lvyouxinde/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
